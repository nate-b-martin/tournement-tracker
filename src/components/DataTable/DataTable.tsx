...
export function DataTable<T extends { _id: string }>({
	...
	const handlePageChange = useCallback(
		...
		[onPaginationChange, paginationState.pageSize],
	);

	const handlePageSizeChange = useCallback(
		(size: number) => onPaginationChange({ pageIndex: 0, pageSize: size }),
		[onPaginationChange],
	);
			...
			<div className="mt-4 space-y-4">
				<div className="rounded-2xl border border-card-outline/70 bg-card/50 p-5 shadow-md">
					...
					<EmptyState
						...
						title={emptyMessage}
						action={
							actions?.canCreate && actions.onCreate
								? {
										label: `Create ${itemName || "item"}`,

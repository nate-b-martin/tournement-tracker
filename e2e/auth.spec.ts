...
test.describe("Authentication", () => {
  test.describe("Unauthenticated", () => {
    ...
    test("redirects from protected route when signed out", async ({ unauthenticatedPage }) => {
    ...
    test("shows access denied message on protected route", async ({ unauthenticatedPage }) => {
      const protectedPage = new ProtectedPage(unauthenticatedPage.page);
      await protectedPage.goto();
      await protectedPage.waitForAccessDenied();
      await expect(protectedPage.accessDeniedTitle).toContainText(/authentication required/i);
    });
  });
